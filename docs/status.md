...
- 76 OpenAPI paths plus one WebSocket transport endpoint
- 233 passing pytest tests, including G12 chord/progression, Vital Pack export,
...
| Instrument-Constrained Composition Explorer | Experimental implementation | Common CompositionGenome for three fractional-pop styles plus ratio-controlled Mixed Style; hierarchical seeds and locks; probabilistic form grammar; temperature-sampled top-K harmony; PI01-PI22 capability constraints including Fractional Piano; generated/imported Mixed Meter drum replacement or layering with metric evaluation; 4-64 candidate deterministic k-medoids; representative audition, variable-meter type-1 pitch-bend MIDI, JSON, and browser-local preference learning at `/composition-explorer` | Rendered-audio evaluation, WAV/stems, transfer into dedicated melodic composers, shared user profiles, automated browser screenshot regression |
...
| Mixed Meter Drum Section | Experimental implementation | Pattern library and validated custom cycles; Stable/Tension/Pre-resolution/Resolved forms; phase timeline; section/tension-aware density; per-track controls; deterministic events and humanization; playback; JSON/type-1 MIDI/WAV; Compose timeline transfer and full-project Composition Explorer integration at `/compose/mixed-meter-drums` | Direct event-to-main-Compose orchestration mapping, interactive hit editing/locks, saved preset library, automated browser regression |
| MIDI Creator Toolkit | Experimental implementation | Web MIDI/channel/CC64 capture; white-key-only one-degree-per-key scale mapping; scale-tuned or 12-TET audio thru; live Pitch Circle with sounding-tone and `3f` third-harmonic layers plus independent octave/tritave wrapping; unified Fractional Pop/J-Pop/Kawaii/series/Bohlen-Pierce catalog; saved Workbench scale loading; Prime-Limit Explorer parameterized scale generation; arbitrary 2/1 or 3/1 equave mapping; quantize, swing, polyphony, transforms; JSON/MPE MIDI; octave-scale Motif Development and Vital Pack handoff at `/midi-toolkit` | MIDI clock, take comping, per-note editing, MIDI file import, MPE controller dimensions, non-octave composer handoff, automated browser regression |
...
| Real-time input and transport | Partial | Web MIDI note/channel/velocity/CC64 capture, exact-ratio scale-tuned monitoring, pointer/PC keyboard fallback, timeline record/replay, WebSocket play/pause/stop/improvise | MIDI clock, MPE controller dimensions, MTS-ESP, OSC, DAW synchronization, MIDI learn |
