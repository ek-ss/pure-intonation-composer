...
このガイドでは、実装済みのすべての機能について説明します。対象は、ブラウザ上のワークベンチ（各パネルの操作方法）および HTTP/WebSocket API です。正確なリクエスト／レスポンスのスキーマについては [api.md](api.md) を、コピーして実行できるコマンドについては [examples.md](examples.md) を参照してください。現在の制約と未実装項目は [status.md](status.md) に集約しています。
