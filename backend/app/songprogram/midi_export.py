...
def export_evaluation_midi(
    ...
    *,
    ...
    for note_event in pitched:
        ...
        available = [channel for channel in pitched_channels if active_until[channel] <= start]
        if not available:
        ...
        role = tracks[note_event["track_id"]]["role"]
        scheduled.setdefault(start, []).extend(
            [
                _event(4, bytes((0xC0 | channel, _PROGRAM_BY_ROLE[role]))),
                _event(5, bytes((0xE0 | channel, bend & 0x7F, bend >> 7))),
                _event(6, bytes((0x90 | channel, note, int(note_event["velocity"])))),
            ]
        )
        scheduled.setdefault(end, []).append(_event(3, bytes((0x80 | channel, note, 0))))
    ...
    for drum in sorted(
        ...
        scheduled.setdefault(start, []).append(
            _event(6, bytes((0x99, note, int(drum["velocity"]))))
        )
        scheduled.setdefault(end, []).append(_event(3, bytes((0x89, note, 0))))

    body = bytearray()
    previous_tick = 0
    for tick in sorted(scheduled):
        first = True
        for _, payload in sorted(scheduled[tick], key=lambda item: (item[0], item[1])):
            body.extend(_vlq(tick - previous_tick if first else 0))
            body.extend(payload)
            previous_tick = tick
            first = False
    body.extend(b"\x00\xff\x2f\x00")
    midi = b"MThd\x00\x00\x00\x06\x00\x00\x00\x01" + ticks_per_beat.to_bytes(2, "big")
    midi += b"MTrk" + len(body).to_bytes(4, "big") + bytes(body)
    ...
    manifest["manifest_hash"] = "sha256:" + hashlib.sha256(
        b"cps.evaluation-reference-midi-manifest/v1\0"
