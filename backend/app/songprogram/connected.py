...
class CompilerReceiptExecutor(Protocol):
    ...
    def __call__(
        self, program: dict[str, Any], compiler_manifest: dict[str, Any]
    ) -> dict[str, Any]: ...
...
def _quote(value: str) -> str:
    ...
    for char in value:
        ...
        if char == '"':
            ...
            chunks.append(escapes[point])
        elif point < 0x20:
            chunks.append(f"\\u{point:04x}")
...
def _encode(value: Any) -> str:
    ...
    if isinstance(value, dict):
        ...
        return (
            "{"
            + ",".join(
                f"{_quote(key)}:{_encode(value[key])}"
...
def artifact_hash(domain: str, value: Any, *, omit: str | None = None) -> str:
    ...
    if omit is not None:
        if not isinstance(core, dict):
            raise ConnectedExecutionError("HASH_INPUT_INVALID", "hash")
        core.pop(omit, None)
...
def cache_key(request: dict[str, Any]) -> dict[str, str]:
    core = {
        ...
        "executor_manifest_digest": executor_manifest_digest(request["executor_manifest"]),
    }
...
def _validate_stream(stream: dict[str, Any]) -> None:
    if (
        stream.get("schema") != "cps.logical-opcode-stream"
        or stream.get("schema_version") != "1.0.0"
    ):
        raise ConnectedExecutionError("CACHE_RECEIPT_INVALID", "opcode_hash")
    if stream.get("stream_hash") != _stream_hash(stream):
        raise ConnectedExecutionError("CACHE_RECEIPT_INVALID", "opcode_hash")
...
def _validate_bundle(bundle: dict[str, Any], receipt: dict[str, Any]) -> None:
    if bundle.get("bundle_hash") != opcode_bundle_hash(bundle):
        raise ConnectedExecutionError("CACHE_RECEIPT_INVALID", "opcode_hash")
    _validate_stream(bundle["root"])
    if bundle["root"]["stream_hash"] != receipt.get("opcode_stream_hash"):
        raise ConnectedExecutionError("CACHE_RECEIPT_INVALID", "opcode_hash")
    children = receipt.get("children", [])
    if len(bundle.get("children", [])) != len(children):
        raise ConnectedExecutionError("CACHE_RECEIPT_INVALID", "opcode_hash")
    for stored, child in zip(bundle["children"], children, strict=True):
        stream = stored.get("stream", {})
        if (stored.get("kind"), stored.get("query_id")) != (
            child.get("kind"),
            child.get("query_id"),
        ):
            raise ConnectedExecutionError("CACHE_RECEIPT_INVALID", "opcode_hash")
        _validate_stream(stream)
        if (stream.get("input_hash"), stream.get("stream_hash")) != (
            child.get("input_hash"),
            child.get("opcode_stream_hash"),
        ):
            raise ConnectedExecutionError("CACHE_RECEIPT_INVALID", "opcode_hash")
...
def _validate_logical_output(output: dict[str, Any], request: dict[str, Any]) -> None:
    ...
    if status == "mutation_failure":
        ...
        if (
            any(
                output.get(key) is not None
                for key in ("project", "compiler_evidence", "melody_report", "lineage_index")
            )
            or output.get("resulting_program") is None
            or output.get("mutation_impact") is None
            or output.get("compile_report") is None
        ):
            raise ConnectedExecutionError("CACHE_RECEIPT_INVALID", "cross_binding")
    elif status == "success":
        if (
            any(
                output.get(key) is None
                for key in (
                    "resulting_program",
                    "mutation_impact",
                    "project",
            ...
            or output.get("lineage_index") is not None
    ...
    if status != "success":
        return
    project, evidence, melody, report = (
        output[key] for key in ("project", "compiler_evidence", "melody_report", "compile_report")
    )
    try:
        project_hash = (
            "sha256:"
        ...
        if (
            ...
            or project_hash != melody["project_hash"]
        ):
            raise KeyError
        if (
            evidence_hash != evidence["evidence_hash"]
            or evidence_hash != report["evidence_hash"]
            or melody_hash != melody["report_hash"]
        ):
            raise KeyError
        if report["receipt"]["input_hash"] is None or report["status"] != "success":
            raise KeyError
...
def build_cache_entry(
    ...
    if output.get("request_hash") != key["request_hash"] or output.get(
        "logical_output_hash"
    ) != logical_output_hash(output):
    ...
    entry = {
        ...
        if report is None
        else _compile_receipt_hash(report["receipt"]),
...
def validate_cache_entry(
    ...
    if entry.get("key") != cache_key(request):
        raise ConnectedExecutionError("CACHE_RECEIPT_INVALID", "requested_key")
    if entry.get("entry_hash") != cache_entry_hash(entry):
        raise ConnectedExecutionError("CACHE_RECEIPT_INVALID", "entry_hash")
    output = entry.get("logical_output")
    if (
        not isinstance(output, dict)
        or entry.get("logical_output_hash") != logical_output_hash(output)
        or output.get("logical_output_hash") != entry.get("logical_output_hash")
    ):
    ...
    if (
        ...
        or entry.get("mutation_receipt_hash") != receipt["receipt_hash"]
    ):
    ...
    if report is None:
        if (
            entry.get("compile_receipt_hash") is not None
            or entry.get("opcode_stream_bundle_hash") is not None
        ):
            raise ConnectedExecutionError("CACHE_RECEIPT_INVALID", "cross_binding")
    else:
        if entry.get("compile_receipt_hash") != _compile_receipt_hash(report["receipt"]):
            raise ConnectedExecutionError("CACHE_RECEIPT_INVALID", "receipt_hash")
        _validate_bundle(bundle, report["receipt"])
        if entry.get("opcode_stream_bundle_hash") != bundle["bundle_hash"]:
            raise ConnectedExecutionError("CACHE_RECEIPT_INVALID", "opcode_hash")
...
class ConnectedCache:
    ...
    def publish(self, request: dict[str, Any], entry: dict[str, Any]) -> None:
        ...
        if self.root is None:
            ...
            if canonical_lf(prior) != canonical_lf(entry):
                raise ConnectedExecutionError("DETERMINISM_VIOLATION", "cache_publish")
            return
        path = self._path(key)
        path.parent.mkdir(parents=True, exist_ok=True)
        ...
        try:
            ...
            raise ConnectedExecutionError("CACHE_PUBLISH_FAILED", "cache_publish") from exc
...
def _mutation_output(request: dict[str, Any], result: dict[str, Any]) -> dict[str, Any]:
    ...
    output = {
        ...
        "mutation_receipt": result["receipt"],
        "project": None,
        "compiler_evidence": None,
        "melody_report": None,
        "compile_report": None,
        "lineage_index": None,
        "error": {key: error[key] for key in ("code", "stage", "pointer")},
        "logical_output_hash": "",
    }
...
def _complete_output(
    request: dict[str, Any], mutation: dict[str, Any], stage: dict[str, Any]
) -> tuple[dict[str, Any], dict[str, Any] | None]:
    status = stage.get("status")
    if status not in {"success", "compile_failure"}:
        raise ConnectedExecutionError("CONNECTED_COMPILER_RESULT_INVALID", "compiler")
    ...
    output = {
        ...
        "mutation_receipt": mutation["receipt"],
        "project": stage.get("project"),
        "compiler_evidence": stage.get("compiler_evidence"),
        "melody_report": stage.get("melody_report"),
        "compile_report": stage.get("compile_report"),
        "lineage_index": None,
        "error": error,
        "logical_output_hash": "",
    }
    ...
    if status == "success":
        if (
            any(
                output[key] is None
                for key in ("project", "compiler_evidence", "melody_report", "compile_report")
            )
        ...
        if (
            any(
                output[key] is not None for key in ("project", "compiler_evidence", "melody_report")
            )
            or output["compile_report"] is None
            or not isinstance(error, dict)
        ):
            raise ConnectedExecutionError("CONNECTED_COMPILER_RESULT_INVALID", "compiler")
    if (output["compile_report"] is None) != (bundle is None):
        raise ConnectedExecutionError("CONNECTED_COMPILER_RESULT_INVALID", "compiler")
    output["logical_output_hash"] = logical_output_hash(output)
...
def validate_connected_request(request: dict[str, Any]) -> None:
    ...
    if request["executor_manifest_digest"] != executor_manifest_digest(
        ...
        raise ConnectedExecutionError("EXECUTOR_MANIFEST_DIGEST_MISMATCH", "request")


def execute_connected(
    request: dict[str, Any],
    compiler: CompilerReceiptExecutor | None,
    cache: ConnectedCache | None = None,
) -> ConnectedExecution:
    ...
    if mutation["status"] == "failure":
        output, bundle = _mutation_output(request, mutation), None
    else:
        if compiler is None:
            ...
            try:
                ...
                raise ConnectedExecutionError(
                    "CONNECTED_COMPILER_RECEIPT_API_UNAVAILABLE", "compiler"
                ) from error
            compiler = compile_connected_gen0b
        output, bundle = _complete_output(
            request, mutation, compiler(mutation["program"], request["compiler_manifest"])
        )
    if cache is not None:
        cache.publish(request, build_cache_entry(request, output, bundle))
    return ConnectedExecution(
        output,
        bundle,
        {
            "lookups": 1 if cache is not None else 0,
            "hits": 0,
            "misses": 1 if cache is not None and not corrupt else 0,
...
def semantic_input_hash(case_pairs: list[dict[str, str]], ceiling: int) -> str:
    return artifact_hash(
        "cps.connected-runner-input/v1",
        {"cases": case_pairs, "global_compile_logical_ceiling": ceiling},
    )


def semantic_runner_request_hash(request: dict[str, Any]) -> str:
    return artifact_hash(
        "cps.connected-runner-semantic-request/v1",
        {key: deepcopy(value) for key, value in request.items() if key != "execution"},
    )


def run_connected_batch(
    request: dict[str, Any],
    ...
    if workers not in (1, 2, 4, 8):
        raise ConnectedExecutionError("RUN_WORKER_COUNT_INVALID", "runner")
    expected = [
        {"case_id": case_id, "connected_request_hash": connected_request_hash(value)}
        for case_id, value in cases
    ]
    if request.get("case_ids") != [item["case_id"] for item in expected] or request.get(
        "semantic_input_hash"
    ) != semantic_input_hash(expected, request.get("global_compile_logical_ceiling")):
    ...
    for ordinal, (case_id, connected) in enumerate(cases):
        ...
        telemetry.append(
            {
                **result.telemetry,
                "schema": "cps.connected-execution-telemetry",
                "schema_version": "1.0.0",
                "cache_mode": execution.get("cache_mode"),
                "worker_count": workers,
                "physical_completed_ordinals": [ordinal],
            }
        )
        charge = (
            0
            if result.output["compile_report"] is None
            else result.output["compile_report"]["receipt"]["usage"]["total_logical_units"]
        )
        ...
        published.append(
            {
                "task_ordinal": ordinal,
                "case_id": case_id,
                "logical_output_hash": result.output["logical_output_hash"],
                "logical_output": result.output,
            }
        )
    result = {
    ...
    result["result_hash"] = artifact_hash(
        "cps.connected-runner-result/v1", result, omit="result_hash"
    )
