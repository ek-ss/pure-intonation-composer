...
OUTPUT_ROOT="${OUTPUT_ROOT:-$ROOT/local_authority/g1_profile_experiments_v1}"
...
for name in baseline contrast_arc motif_recall rhythm_dialogue; do
  if [[ "$name" == baseline ]]; then
    profile="$PROFILES/composition_generation_v2.json"
  else
    profile="$PROFILES/g1_experiments/$name.json"
  fi
  command=(
