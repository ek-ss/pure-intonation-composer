"""Generate checked-in five-dimensional GEN0-B v2 fixtures without production code."""
...
from .identifiers import (budget_profile_digest, compiler_build_id, event_id,
    harmony_occurrence_id, program_hash, project_artifact_hash, semantic_address, lattice_domain_hash)
...
OUT = ROOT / "fixtures" / "compiler_v2_5d"
...
def build_5d_inputs() -> tuple[dict, dict, dict, dict]:
    ...
    base_project["schema_version"] = "1.3.0"
    base_project["source_program"]["schema_version"] = "0.2.0"
    base_project["lattice"]["generators"] += ["7/1", "11/1", "13/1"]
    base_project["lattice"]["coordinate_bounds"] += [[0, 0], [0, 0], [0, 0]]
    base_project["lattice"]["domain_hash"] = lattice_domain_hash(base_project["lattice"])
    for base_chord in base_project["resolved_chords"]:
        base_chord["domain_hash"] = base_project["lattice"]["domain_hash"]
        base_chord["anchor_vector"] += [0, 0, 0]
        base_chord["voice_offsets"] = [vector + [0, 0, 0] for vector in base_chord["voice_offsets"]]
    ...
    resolver_profile={"schema":"cps.resolver-profile","schema_version":"1.0.0","algorithm":"sp0-joint-bnb/v1","resolver_build_id":"gen0a-bnb-reference-oracle-v1","candidates_per_intent":24,"optimized_report_schema_version":"1.0.0","progression_algorithm":"gen0-progression-exact/v1","operation_budget_profile":"gen0-progression-exact-v2"}
    resolver_profile["profile_hash"]=artifact_hash("cps.resolver-profile/v1",resolver_profile)
    manifest=deepcopy(base_manifest); manifest["schema_version"]="2.0.0"
    manifest["song_program_schema_versions"]=["0.2.0"]
    manifest["project_schema_version"]="1.3.0"
    manifest["progression_query_schema_version"]="2.0.0"
    manifest["gen0b_evidence_schema_version"]="2.0.0"
    manifest["melody_report_schema_version"]="2.0.0"
    ...
    profile=manifest["budget_profile"]; profile["id"]="gen0-progression-exact-v2"
    profile["domain_limits"]["dimensions"]=5
    ...
    program["program_id"] = "sp_gen0b_melody_5d"
    ...
    program["materials"][1]["root_anchors"] = [[0, 0, 0, 0, 0]]
...
def compile_pipeline(program: dict, manifest: dict, base_project: dict) -> dict:
    ...
    candidate = {"core_hash":core_hash,"resolved_chord":chord,"domain_hash":chord["domain_hash"],"intent_hash":chord["intent_hash"],"anchor_vector":[0,0,0,0,0],"voices":voices,"local_pair_rms_millicents":chord["pair_rms_error_millicents"],"local_pair_max_millicents":chord["maximum_pair_error_millicents"],"local_complexity":chord["complexity_score"]}
    occ_ids = [harmony_occurrence_id("sec_a","real_harmony",0,"harmony_a",i) for i in range(3)]
    query = {"schema":"cps.progression-query","schema_version":"2.0.0","algorithm":"gen0-progression-exact/v1","numeric_contract":"cps-numeric/decimal-log2-rhe-v1","budget_profile":"gen0-progression-exact-v2","domain_hash":chord["domain_hash"],"domain_equave":"2/1","maximum_voice_motion_millicents":2400000,"crossing_policy":"forbid","occurrences":[]}
    for i, oid in enumerate(occ_ids):
        query["occurrences"].append({"id":oid,"start_tick":i*640,"duration_ticks":640,"track_id":"harmony","register_millicents":[-1200000,3600000],"maximum_polyphony":4,"overlapping_nonprogression_pitched_events":0,"intent_hash":chord["intent_hash"],"root_anchor":[0,0,0,0,0],"candidate_cores":[candidate]})
    ...
    occurrences=[{"occurrence_id":occ_ids[i],"section_id":"sec_a","track_id":"harmony","realization_id":"real_harmony","repeat_ordinal":0,"material_id":"harmony_a","source_step_ordinal":i,"start_tick":i*640,"duration_ticks":640,"intent_id":"ci_major","intent_hash":chord["intent_hash"],"root_anchor":[0,0,0,0,0]} for i in range(3)]
    query_hash=artifact_hash("cps.progression-query/v2",query); result_hash=artifact_hash("cps.progression-result/v1",result)
    bindings=[]
    for i in range(3):
    ...
    oracle_query={"schema":"cps.sp0-oracle-query/v1","numeric_contract":manifest["numeric_contract"],"domain":domain,
        ...
        "anchor":{"vector":[0,0,0,0,0],"equave_exponent":0}}
    chord_query_hash=_bare_hash(oracle_query); chord_child_id="chord_"+chord_query_hash[7:23]
    stats=trace_bnb(emitter,program["lattice"],program["chord_intents"][0],[0,0,0,0,0],chord_child_id,1,(-1200000,3600000))
    ...
    progression_query_hash=artifact_hash("cps.progression-query/v2",query)
    ...
    receipt={"schema":"cps.charge-receipt","schema_version":"1.1.0","budget_profile_id":"gen0-progression-exact-v2",
    ...
    manifest_hash=artifact_hash("cps.compiler-manifest/v2.0",manifest)
    evidence={"schema":"cps.gen0b-compiler-evidence","schema_version":"2.0.0","source_program_hash":program_hash(program),
        ...
        "gen0a_results":[{"query_hash":chord_query_hash,"domain_hash":chord["domain_hash"],"intent_hash":chord["intent_hash"],
            "root_anchor":[0,0,0,0,0],"requested_k":1,"ordered_core_hashes":[core_hash],"child_receipt_digest":_child_receipt(chord_child)}],
    ...
    evidence["evidence_hash"]=artifact_hash("cps.gen0b-compiler-evidence/v2",evidence)
    melody_report={"schema":"cps.chord-member-melody-report","schema_version":"2.0.0","source_program_hash":program_hash(program),
        "gen0b_evidence_hash":evidence["evidence_hash"],"project_hash":phash,"bindings":bindings}
    melody_report["report_hash"]=artifact_hash("cps.chord-member-melody-report/v2",melody_report)
...
def main() -> None:
    ...
    negative_cases={"schema":"cps.gen0b-5d-negative-cases","schema_version":"1.0.0","cases":[
        ...
        {"id":"placed_cardinality_4200_rejected","domain":{"dimensions":5,"axis_width":7,"coordinate_cardinality":700,"register_width":6,"placed_cardinality":4200},"expected_error":"GEN0B_DOMAIN_LIMIT_EXCEEDED"},
    ]}
    _write("gen0b_5d_negative_cases.json",negative_cases)
    files=[name for name in sorted(p.name for p in OUT.glob("*.json")) if name != "gen0b_melody_fixture_set.json"]
    fixture={
        "schema":"cps.gen0b-5d-fixture-suite-index","schema_version":"1.0.0",
        "authority_status":"authoritative","generator_identity":"cps-independent-gen0b-5d-oracle/1.0.0",
        "contract":"cps-gen0b-5d-authoritative/v1",
        ...
        "schema_sha256":{
            ...
            for name in (
                ...
                "gen0b_compiler_evidence_2_0.schema.json",
                "chord_member_melody_report_2_0.schema.json",
                "gen0b_5d_fixture_suite_index.schema.json",
            )
        },
        "execution":{"worker_counts":[1,2,4,8],"pythonhashseeds":["0","1","7","42"]},
        "suite_hash":"",
    }
    fixture["suite_hash"]=artifact_hash("cps.gen0b-5d-fixture-suite-index/v1",{key:value for key,value in fixture.items() if key!="suite_hash"})
